...
function getCoords(searchInput) {
    ...
    // if search is by city, return first result
        ...
        var searchApiUrl = `http://api.openweathermap.org/geo/1.0/direct?q=${searchInput}&limit=0&appid=${WEATHER_API_KEY}`
        fetch(searchApiUrl, {
            method: 'GET',
...
function getForecast(lat,lon) {
    ...
    .then(data => {
        ...
        var resultsWeatherIcon = data.list[0].weather[0].icon;
        var resultsWeatherIconUrl = `http://openweathermap.org/img/wn/${resultsWeatherIcon}.png`
        ...
        resultsCityEl.text(resultsCity).append(today);
        resultsIconEl.attr('src', resultsWeatherIconUrl);
        resultsTempEl.text('Temp: ' + Math.trunc(resultsTemp) + '° F') ;
